...
export async function enterRescue(world,state=rescueInitial()){
 ...
 const [asset,garments,regions,usMapSvg]=await Promise.all([new GLTFLoader().setMeshoptDecoder(MeshoptDecoder).loadAsync('/assets/rescue/rescue-room.glb'),fetch('/assets/rescue/catalog.json').then(r=>r.json()),fetch('/assets/rescue/regions.json').then(r=>r.json()),fetch('/assets/rescue/us-map.svg').then(r=>{if(!r.ok)throw Error('미국 상세 지도를 불러올 수 없어요.');return r.text();})]);
 const old=world.scene,scene=new THREE.Scene();scene.background=new THREE.Color('#a6b7c1');scene.environment=old.environment;scene.environmentIntensity=.36;scene.fog=new THREE.Fog('#d8e7f1',48,85);
 world.scene=scene;world.model=asset.scene;scene.add(asset.scene,world.camera,world.avatar);world.mode='rescue-loading';world.state={stage:0,inventory:[]};world.targets=[];world.keys.clear();world.hit=null;world.ride=null;world.pendingAnimations=[];world.avatarPose='standing';world.camera.up.set(0,1,0);world.camera.far=90;world.camera.updateProjectionMatrix();world.renderer.toneMappingExposure=1;world.renderer.shadowMap.enabled=true;world.renderer.shadowMap.type=THREE.PCFSoftShadowMap;
 const hemi=new THREE.HemisphereLight('#ffffff','#526376',.78);scene.add(hemi);const sun=new THREE.DirectionalLight('#fffaf4',2.05);sun.position.set(0,7,3);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.camera.left=-7;sun.shadow.camera.right=7;sun.shadow.camera.top=7;sun.shadow.camera.bottom=-7;sun.shadow.bias=-.001;sun.shadow.normalBias=.035;scene.add(sun,sun.target);const fill=new THREE.DirectionalLight('#dfedff',.4);fill.position.set(-5,3,-6);scene.add(fill);
 ...
 await addInteriorDetails(world,'rescue');polishRoomMaterials(asset.scene,'rescue');world.renderer.shadowMap.needsUpdate=true;
 // Transparent panes must neither occlude the trophies in the depth buffer nor cast solid shadows.
 asset.scene.traverse(o=>{if(!o.isMesh)return;const materials=Array.isArray(o.material)?o.material:[o.material];if(materials.some(m=>/^Cabinet (clear glass|polished glass edges)/.test(m.name))){o.castShadow=false;o.receiveShadow=false;for(const m of materials){m.transparent=true;m.depthWrite=false;m.side=THREE.DoubleSide;}o.renderOrder=2;}});
 await installHyunsu(asset.scene,['HyunsuBed','HyunsuCold','HyunsuHungry','HyunsuQuake']);
 ...
 world.label('HYUNSU  /  REST & RECOVER',[1.7,2.65,-4.69],2.25,30,'#eef7f8');const coldLabel=world.label('바람이 너무 차가워…',[8.7,2.25,1.2],2.3,30,'#233a56'),warmLabel=world.label('이제 따뜻해! 고마워, 하영아 ♡',[8.7,2.35,1.2],3,32,'#80563d');warmLabel.visible=state.warmed;
 ...
 const collision=[...detailColliders.rescue,[-4.4,-1.35,-4.8,-3.82],[.35,3,-3.85,-.3],[3.5,4.3,-3.2,-2.4],[10,17,-2.75,.4],[13.5,16.8,2.3,3.4],[22,24.8,.1,1.75],[40,41.3,3.3,4.3]];
