...
import {stations,ROOM_RADIUS,WALK_RADIUS,kitchenArchitecture,kitchenWalls,powerPosition,entryPosition,exitPosition,courierStart,courierEnd,storeColliders,marketTargets} from './kitchen-layout.js';
...
function foodSource(key,progress){
 const stages={kimchi:['RiceOnly','RiceOnly','Pan_kimchi','Pan_kimchi','Pan_kimchi','FriedRice','Dish_kimchi'],pepero:['WaterPot','WaterPot','ChocolateChunks','ChocolateBowl','PeperoCoated','Dish_pepero','Dish_pepero','Dish_pepero'],chili:['Pan_raw','Pan_raw','Pan_raw','Pan_cooked','Pan_cooked','Pan_sauced','Dish_chili'],chicken:['GiftParcel','GiftParcel','GiftParcel','ChickenEmpty','ChickenRice','ChickenFried','Dish_chicken','Dish_chicken']};
...
export async function enterKitchen(world,state){
 ...
 polishRoomMaterials(asset.scene,'kitchen');
 ...
 world.camera.up.set(0,1,0);world.camera.far=80;world.camera.updateProjectionMatrix();world.renderer.shadowMap.enabled=false;world.renderer.toneMappingExposure=1;
 await addInteriorDetails(world,'kitchen');
 const hemi=new THREE.HemisphereLight('#ffffff','#4f6274',0),sun=new THREE.DirectionalLight('#ffffff',0);sun.position.set(6,6,8);scene.add(hemi,sun);
 ...
 for(const key of recipeKeys){target('cook-'+key,recipes[key].name,recipes[key].clock+' 요리방 · 레시피와 조리대',stationPoint(key,0,1.6,-.9),[4,2,3]);target('kitchen-door-'+key,recipes[key].name+' 문','E · 요리방 열기',stationPoint(key,0,1.7,2.65),[2.6,3.3,.5]);}
 ...
 const roomDoors={};const doorWood=new THREE.MeshStandardMaterial({color:'#79503a',roughness:.48});const doorGlass=new THREE.MeshStandardMaterial({color:'#b8dec8',transparent:true,opacity:.48,roughness:.14,metalness:.1});
 for(const key of recipeKeys){const frame=new THREE.Group();frame.position.set(...stationPoint(key,0,0,kitchenArchitecture.doorPlane));frame.rotation.y=stations[key][2];scene.add(frame);doorPart(-1.25,1.75,0,.12,3.5,.30,brass,frame);doorPart(1.25,1.75,0,.12,3.5,.30,brass,frame);doorPart(0,3.52,0,2.6,.24,.30,brass,frame);const hinge=new THREE.Group();hinge.position.x=-1.19;frame.add(hinge);doorPart(1.19,1.72,0,2.34,3.36,.105,doorWood,hinge);doorPart(1.19,2.05,.07,1.65,1.36,.018,doorGlass,hinge);doorPart(2.12,1.65,.14,.09,.28,.1,brass,hinge);roomDoors[key]=hinge;}
 const labels=[];
 ...
 for(const key of recipeKeys){const [cx,cz,a]=stations[key];if(Math.abs(Math.sin(a))<.5)world.colliders.push([cx-3.1,cx+3.1,cz+(a===0?-2.52:.88),cz+(a===0?-.88:2.52)]);else world.colliders.push([cx+(cx>0?.88:-2.52),cx+(cx>0?2.52:-.88),cz-3.1,cz+3.1]);}
 for(const {center:[x,y,z],size:[w,h,d]} of kitchenWalls)if(y-h/2<1.63)world.colliders.push([x-w/2,x+w/2,z-d/2,z+d/2]);
 const baseColliders=world.colliders.map(box=>[...box]);
 world.wallBoxes=kitchenWalls.map(({center,size})=>new THREE.Box3().setFromCenterAndSize(v(center),v(size)));
 // Continuous polygon segments stop the third-person camera at the visible shell.
 // Point-sized boxes left large gaps even though the player could not leave the room.
 for(let i=0;i<96;i++){const points=[];for(const a of [i*Math.PI/48,(i+1)*Math.PI/48])for(const r of [ROOM_RADIUS-.2,ROOM_RADIUS])for(const y of [0,kitchenArchitecture.ceilingHeight])points.push(v([r*Math.cos(a),y,r*Math.sin(a)]));world.wallBoxes.push(new THREE.Box3().setFromPoints(points));}
 world.wallBoxes.push(new THREE.Box3(v([-ROOM_RADIUS,-.22,-ROOM_RADIUS]),v([ROOM_RADIUS,.02,ROOM_RADIUS])),new THREE.Box3(v([-ROOM_RADIUS,kitchenArchitecture.ceilingHeight,-ROOM_RADIUS]),v([ROOM_RADIUS,7.94,ROOM_RADIUS])));
 function updateDoorColliders(s){world.colliders=baseColliders.map(box=>[...box]);for(const key of recipeKeys)if(!s.opened?.[key]){const [x,,z]=stationPoint(key,0,0,kitchenArchitecture.doorPlane),east=Math.abs(Math.sin(stations[key][2]))>.5;world.colliders.push(east?[x-.18,x+.18,z-1.2,z+1.2]:[x-1.2,x+1.2,z-.18,z+.18]);}world.collisionBoxes=[...world.wallBoxes,...world.colliders.map(([a,b,c,d])=>new THREE.Box3(v([a,0,c]),v([b,3.5,d])))];}
 updateDoorColliders(state);
 const prototypes={};for(const name of ['Dish_kimchi','Dish_pepero','Dish_chili','Dish_chicken','Pan_kimchi','Pan_raw','Pan_cooked','Pan_sauced','ChocolateBowl','GiftParcel','ChickenEmpty','ChickenRice','ChickenFried','PeperoCoated','RiceOnly','FriedRice','SideShrimp','WaterPot','ChocolateChunks']){prototypes[name]=world.get(name);prototypes[name].visible=false;}
 ...
 const effects=Object.fromEntries(['kimchi','chili','pepero'].map(key=>[key,new CookingEffects(scene,stationPoint(key,.1,1.31,-1.65))]));
 ...
 const runtime={prototypes,displays,courier,effects,state,setHeat(key,heat){effects[key]?.setHeat(heat);},lastVersion:'',lightAmount:state.lights?1:0,arrivalSound:false,handoffAt:null,
  sync(s){if(s.received&&!this.state.received)this.handoffAt=Date.now();this.state=s;updateDoorColliders(s);const signature=JSON.stringify(s.progress);if(this.lastVersion!==signature){for(const key of recipeKeys){const group=displays[key];group.clear();
  ...
  update(dt,now){this.lightAmount=THREE.MathUtils.damp(this.lightAmount,this.state.lights?1:0,2.3,dt);const f=this.lightAmount;hemi.intensity=.9*f;sun.intensity=1.7*f;fill.intensity=.45*f;scene.environmentIntensity=.38*f;scene.background.setRGB(.008+.19*f,.012+.22*f,.011+.17*f);labels.forEach(l=>l.visible=f>.1);world.dust.visible=f>.2;emissive.forEach(([m,strength])=>m.emissiveIntensity=strength*f);for(const fx of Object.values(effects)){fx.root.visible=f>.2;fx.update(dt);}
   ...
   for(const [index,key] of recipeKeys.entries()){roomDoors[key].rotation.y=THREE.MathUtils.damp(roomDoors[key].rotation.y,this.state.opened?.[key]?-1.65:0,3.4,dt);const allowed=index===0||this.state.progress[recipeKeys[index-1]]===recipes[recipeKeys[index-1]].steps.length;doorLabels[key].ready.visible=f>.1&&allowed&&!this.state.opened?.[key];doorLabels[key].locked.visible=f>.1&&!allowed&&!this.state.opened?.[key];}
   world.targets.forEach(t=>{const id=t.userData.id;t.visible=id==='kitchen-power'||this.state.lights;if(id.startsWith('kitchen-door-'))t.visible=this.state.lights&&!this.state.opened?.[id.slice(13)];if(id.startsWith('cook-'))t.visible=this.state.opened?.[id.slice(5)]===true;if(id==='kitchen-courier')t.visible=this.state.orderAt!==null&&now>=this.state.orderAt+10000&&!this.state.received;});
