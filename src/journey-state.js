...
export const initialJourney=()=>({version:1,zone:'hell',chat:false,freed:false,photo:false,computer:false,ticket:false,bowl:false,served:false,seat:null,flight:0,arrived:false,dining:false,chosen:[],rejected:[],satisfied:false,mealShared:false,reminisced:false,tea:false,drank:false,cupHeard:false,cupInspected:false,key:false,completed:false});
export function loadJourney(raw){
 ...
 s.satisfied=s.dining&&s.chosen.length===5&&s.satisfied;
 // Earlier saves already holding tea or the key retain their earned progress.
 s.mealShared=s.satisfied&&(raw.mealShared===true||s.tea);
 s.reminisced=s.mealShared&&(raw.reminisced===true||s.tea);
 s.tea=s.reminisced&&s.tea;s.drank=s.tea&&s.drank;
 s.cupHeard=s.drank&&(raw.cupHeard===true||s.key);
 s.cupInspected=s.cupHeard&&(raw.cupInspected===true||s.key);
 s.key=s.cupInspected&&s.key;s.completed=s.key&&s.completed;
...
export function journeyTransition(current,e){
 ...
 case 'serve':if(s.zone!=='church'||!s.bowl||s.served)return deny('승무원이 길을 막는다. “배가 고파서 움직이기 싫어요…”');s.bowl=false;s.served=true;message='“감사합니다! 탑승문을 열어 드릴게요. 문 안으로 걸어오세요.”';effect='success';break;
 ...
 case 'pick':{if(!s.dining||s.zone!=='restaurant'||s.satisfied)return deny('먼저 현수와 함께 자리에 앉아 줘.');const food=menu.find(m=>m.id===e.id);if(!food)return deny('레일에 없는 메뉴야.');if([...s.chosen,...s.rejected].includes(food.id))return deny('한 번 집은 메뉴는 다시 가져올 수 없어.');if(!food.liked){s.rejected.push(food.id);message=`“${food.name}은 내가 안 좋아하는 거야…” 현수가 접시를 옆으로 밀어 두었다. 식탁에는 올리지 않는다.`;effect='error';}else{s.chosen.push(food.id);message=`${food.name}을 건졌어. ${s.chosen.length} / 5`;if(s.chosen.length===5){s.satisfied=true;message='다섯 접시가 모였어. 이제 현수와 천천히 먹어 보자.';effect='success';}}break;}
 case 'share-meal':if(s.zone!=='restaurant'||!s.satisfied||s.mealShared)return deny('좋아하는 메뉴를 모두 고른 뒤 함께 먹어 줘.');s.mealShared=true;message='보글보글 끓는 냄비 앞에서 둘만의 식사가 시작됐다.';break;
 case 'reminisce':if(s.zone!=='restaurant'||!s.mealShared||s.reminisced)return deny('현수와 식사를 하며 이야기를 나눠 봐.');s.reminisced=true;message='한참 이야기를 나누던 현수가 가방에서 차를 꺼냈다.';break;
 case 'tea':if(s.zone!=='restaurant'||!s.reminisced||s.tea)return deny('함께 식사하고 현수의 이야기를 들어 줘.');s.tea=true;message='현수가 ZAGEE 차를 건넸어.';break;
 case 'drink':if(s.zone!=='restaurant'||!s.tea||s.drank)return deny('먼저 현수가 주는 차를 받아 줘.');s.drank=true;message='차의 향이 퍼지고, 매콤했던 입안이 부드러워진다.';break;
 case 'hear-cup':if(s.zone!=='restaurant'||!s.drank||s.cupHeard)return deny('차를 한 모금 마시고 컵을 내려놓아 봐.');s.cupHeard=true;message='달그락. 얼음 소리와는 조금 다른 소리가 들렸다.';break;
 case 'inspect-cup':if(s.zone!=='restaurant'||!s.cupHeard||s.cupInspected)return deny('컵에서 들려온 낯선 소리부터 확인해 봐.');s.cupInspected=true;message='뚜껑을 열자, 컵 안에서 작은 금빛 열쇠가 반짝였다.';break;
 case 'key':if(s.zone!=='restaurant'||!s.cupInspected||s.key)return deny('뚜껑을 열고 컵 안을 먼저 살펴봐.');s.key=true;message='컵 속에서 500일로 향하는 열쇠를 찾았어.';effect='hatch';break;
...
export function journeyObjective(s){return s.completed?'500일로 이어지는 문이 열렸어.':s.key?'열쇠로 500일을 향하는 문을 열어 줘.':s.cupInspected?'컵 안에서 발견한 작은 열쇠를 꺼내 봐.':s.cupHeard?'낯선 소리가 난 컵의 뚜껑을 열어 봐.':s.drank?'차의 여운을 느끼며 컵을 살짝 내려놓아 봐.':s.tea?'현수가 준 ZAGEE를 한 모금 마셔 봐.':s.reminisced?'현수가 선물하는 차를 받아 줘.':s.mealShared?'현수와 오늘의 여행 이야기를 나눠 봐.':s.satisfied?'다섯 접시가 모였어. 현수와 천천히 식사해 줘.':s.dining?`현수가 좋아하는 서로 다른 메뉴 다섯 개. ${s.chosen.length} / 5`:s.zone==='restaurant'?'레드카펫 계단을 내려가 현수와 자리에 앉아 줘.':s.arrived?'도착했어. 출구로 걸어가 비행기에서 내려 줘.':s.seat?'연태로 비행 중… 곧 도착합니다.':s.zone==='plane'?'우리 둘뿐인 퍼스트 클래스. 어느 자리든 골라 앉아 줘.':s.served?'웃는 승무원의 안내를 받아 비행기에 올라 줘.':s.bowl?'육회비빔밥을 배고픈 승무원에게 건네 줘.':s.ticket?'탑승권의 게이트와 좌석을 따라가 봐.':s.computer?'하영이의 휴대폰 카메라로 컴퓨터의 QR을 비춰 봐.':s.photo?'사진 속 식당 이름으로 컴퓨터를 열어 줘.':s.freed?'현수가 있던 감옥 안의 사진을 살펴봐.':'책상의 휴대폰에 남은 대화로 현수의 감옥을 열어 줘.';}
