...
async function start(fixture){await page.goto('http://127.0.0.1:5179/?e2e');await page.evaluate(s=>localStorage.setItem('hayoung500.room1.v1',JSON.stringify(s)),fixture);await page.reload();await page.locator('#continue').click();await page.waitForFunction(()=>window.__roomTest?.ready,null,{timeout:120000});}
...
try{
 ...
 await page.waitForFunction(()=>window.__roomTest.view().mode==='kitchen',null,{timeout:60000});await page.waitForTimeout(500);await shot('transfer-07-room-two');assert.equal(await page.evaluate(()=>window.__roomTest.read().location),'kitchen');checks.push('Ride completes and saves arrival in the dark 200-day cooking room');
 ...
 await page.evaluate(()=>{window.__roomTest.setPosition(14.9,1);window.__roomTest.look(20,1.63,1);});await page.keyboard.down('KeyW');await page.waitForTimeout(500);await page.keyboard.up('KeyW');assert.ok(Math.hypot((await view()).position[0],(await view()).position[2])<=15.31);checks.push('Circular wall collision and arrival walking work');
 await page.reload();await page.locator('#continue').click();await page.waitForFunction(()=>window.__roomTest?.view().mode==='kitchen',null,{timeout:120000});checks.push('Reload resumes in room two without replaying ride');
