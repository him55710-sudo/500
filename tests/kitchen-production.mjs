...
try{
 ...
 for(const file of ['food-reference/chili.jpg','food-reference/chicken.jpg','food-reference/garlic.jpg','food-reference/pepero.jpg','food-reference/CREDITS.md','market-icons/carrot.png','market-icons/LICENSE.txt']){const response=await page.request.get(new URL('/assets/'+file,url).href);assert.equal(response.status(),200);assert.equal(hash(await response.body()),hash(await fs.readFile('public/assets/'+file)));}
 assert.deepEqual(errors,[]);
 await fs.writeFile('test-results/kitchen-public-report.json',JSON.stringify({ok:true,url,checks:['Real controls open the dark-room map','Confirmed station order turns on lights','Third-person view and hints work','Preview preserves previous save','Five uploaded images match source bytes','Compressed kitchen model matches deployment','Four new food photographs, model icon and both license files match source bytes'],errors},null,2));
